module VotesHelper
  ...
  def voting(votable)
    content_tag(:div, class: 'btn-group-vertical', id: "voting_#{votable.class.to_s.downcase}_#{votable.id.to_s}") do
      ...
      content_tag(:div, votes(votable), class: 'btn-group') +
  ...
  def vote_up(votable)
    link_to votable_votes_path(votable, vote: 'up'), method: :post, remote: true do
      content_tag(:i, nil, class: 'fa fa-chevron-up fa-lg')
    end
  end

  def vote_down(votable)
    link_to votable_votes_path(votable, vote: 'down'), method: :post, remote: true do
      content_tag(:i, nil, class: 'fa fa-chevron-down fa-lg')
    end
  end

  def votes(votable)
    content_tag(:h4, votable.get_upvotes.size - votable.get_downvotes.size)
  end
